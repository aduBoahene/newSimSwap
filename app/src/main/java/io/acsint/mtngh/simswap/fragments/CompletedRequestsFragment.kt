...
class CompletedRequestsFragment() : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        CompletedRequestFetcherTask(activity!!.applicationContext).execute()
    ...
    inner class CompletedRequestFetcherTask(var context: Context) : AsyncTask<Void, Void, List<SimResponse.SwapRequest>>() {
        lateinit var errorMessage: String

        override fun doInBackground(vararg params: Void?): List<SimResponse.SwapRequest> {
            return showCompletedSwapedSimRequests()
        }
        ...
        public fun showCompletedSwapedSimRequests(): List<SimResponse.SwapRequest> {
            try {
                errorMessage = "";

                var simRequestResponse = swapList()
                        .completedSimswaplist(context.getAuthToken(), context.getAgentId()).execute()
